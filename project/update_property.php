...
if(isset($_POST['update'])){
 ...
 #Map Link
   if(empty($map_link)){
    $warning_msg[] = 'Map link is required!';
   }elseif (!filter_var($map_link, FILTER_VALIDATE_URL)) {
    $warning_msg[] = 'Invalid map link!';
   }
   if(isset($_POST['lift'])){
      $lift = $_POST['lift'];
   ...
   $update_listing = $conn->prepare("UPDATE `property` SET property_name = ?, address = ?, map_link = ?, price = ?, type = ?, offer = ?, status = ?, furnished = ?, bhk = ?, deposite = ?, bedroom = ?, bathroom = ?, carpet = ?, age = ?, total_floors = ?, room_floor = ?, loan = ?, lift = ?, security_guard = ?, play_ground = ?, garden = ?, water_supply = ?, power_backup = ?, parking_area = ?, gym = ?, shopping_mall = ?, hospital = ?, school = ?, market_area = ?, description = ? WHERE id = ?");   
   $update_listing->execute([$property_name, $address, $map_link, $price, $type, $offer, $status, $furnished, $bhk, $deposite, $bedroom, $bathroom, $carpet, $age, $total_floors, $room_floor, $loan, $lift, $security_guard, $play_ground, $garden, $water_supply, $power_backup, $parking_area, $gym, $shopping_mall, $hospital, $school, $market_area, $description, $update_id]);
...
<section class="property-form">
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      ...
      <div class="flex">
         ...
         <div class="box">
            <p>property map link <span>*</span></p>
            <input type="text" name="map_link" required maxlength="500" placeholder="enter map link..." class="input" value="<?= $fetch_property['map_link']; ?>">
         </div>
         <div class="box">
         ...
         <div class="box">
            ...
            <select name="type" required class="input">
               ...
               <option value="house">pg hostels</option>
               <option value="house">marriage halls</option>
               <option value="house">conference room</option>
         ...
         <div class="box">
            ...
            <select name="bathroom" required class="input">
               ...
               <option value="1">0 bathroom</option>
